...
export interface ExportButtonProps {
  /**
   * API endpoint path for live PDF generation (server-side with Puppeteer)
   * Set to undefined/null to disable live mode
   * @default '/api/export-pdf'
   */
  apiPath?: string | null;

  /**
   * Path to the manifest.json generated by the CLI.
   * When set, the button will check if a pre-rendered PDF exists and serve it as a static download.
   * If both manifestPath and apiPath are set, manifest is tried first with live as fallback.
   * @default undefined
   */
  manifestPath?: string;
  ...
  /**
   * Hide the button entirely when no PDF is available (manifest mode only, no apiPath fallback)
   * @default false
   */
